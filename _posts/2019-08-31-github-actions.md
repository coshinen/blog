...
GitHub Actions 给你提供了灵活构建一个自动化的软件开发生命周期的工作流。
你可以编写单独的任务，调用操作，再合并它们以创建一个自定义的工作流。
工作流时自定义自动化的进程，你可以在仓库中设置来构建、测试、打包、发布或部署 GitHub 上的任何代码项目。

通过 GitHub Actions 你可以直接在仓库中构建端到端的持续集成和持续部署功能。
GitHub Actions 为 GitHub 内置的持续集成（CI）服务提供动力。

工作流在 GitHub 托管的服务器上的 Linux、macOS、Windows 和容器中运行。
你可以使用仓库定义的操作、在 GitHub 上的一个公共仓库中的开源操作或一个已发布的 Docker 容器镜像来创建工作流。

你可以在 GitHub 上找到用于你工作流的操作，并构建 GitHub 社区共享的操作。

你可以创建一个配置运行在特定事件上的工作流文件。

## 1. GitHub Actions 的核心操作

这是一些使用 GitHub Actions 时具有特定含义的常用术语。

### 1.1. 工作流

一个可配置的自动化进程，你可以在你的仓库中设置该流程来在 GitHub 上构建、测试、打包、发布或部署任何项目。
工作流由一个或多个工作（jobs）组成，并可以通过一个事件安排或激活。

### 1.2. 工作流运行

一个当预配置的事件发生时运行的工作流实力。
你可以看到每个工作流运行的工作（jobs）、操作、日志和状态。

### 1.3. 工作流文件

使用至少一个工作（jobs）定义工作流配置的 YAML 文件。
...
作为创建一个工作的步骤（steps）组合的单个任务。
操作是一个工作流中最小的可移植构建块。
你可以创建你自己的操作，使用 GitHub 社区共享操作并自定义公共操作。
若要在一个工作流中使用一个操作，你必须将其作为一个步骤包含进去。

### 1.7. 持续集成（CI）

频繁地向一个共享仓库提交小量的代码更改的软件开发实践。
使用 GitHub Actions，你可以创建自定义的 CI 工作流，以自动构建并测试你的代码。
从你的仓库中，你可以查看代码更改的状态和工作流中每个操作的详细日志。
CI 通过提供代码更改的及时反馈来更快地检测并解决 bugs，从而节省开发人员的事件。

### 1.8. 持续部署（CD）

持续部署建立在持续集成的基础上。
当提交新代码和通过你的 CI 测试时，代码将自动部署到生产环境中。
使用 GitHub Actions，你可以创建自定义的 CD 工作流，以便从你的仓库自动部署代码到任何云、自托管服务或平台。
CD 通过自动化部署过程节省开发人员的时间，并更快地向你的客户部署经过测试的、稳定的代码更改。
...
每个虚拟环境中等待可用工作（jobs）的一个 GitHub 服务。
当一个运行器拿起一个工作时，它会运行该工作的操作并报告进度、日志和最终结果到 GitHub。
运行器一次只运行一个工作。
